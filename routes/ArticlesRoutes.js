...
async function getArticlesFromBC() {
  ...
  const url = `http://${process.env.SERVER}:7048/BC210/ODataV4/Company('${encodedCompanyId}')/ItemListeReduit`;
...
router.get("/", async (req, res) => {
  try {
    ...
    console.error(
      "Error retrieving articles data from Business Central:", error.message);
  ...
  try {
    ...
    const articleDocs = articles.map((article) => {
      ...
      return {
        ...
        numFournisseur: article.N_x00B0__Fournisseur,
